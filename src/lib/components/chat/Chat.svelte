<script lang="ts">
	...
	$: {
		...
		if (nextChatIdProp !== previousChatIdProp) {
			if (!nextChatIdProp && previousChatIdProp && (projectBaseUrl || embedded)) {
				void initNewChat();
			}
	...
	// `readOnly` (prop) forces read-only for a submitted assignment; upstream's own rule
	// covers viewing someone else's chat.
	$: chatReadOnly = readOnly || (chat != null && chat.user_id !== $user?.id);
	...
	const loadChat = async () => {
		...
		if (chat) {
			// An assignment conversation belongs to its writing workspace: opening it as a
			// plain chat would bypass the workspace's provenance tracking, so bounce it back.
			if (!allowAssignmentWorkspaceChat) {
				const guardFolder = chat?.folder_id
					? await getFolderById(localStorage.token, chat.folder_id).catch(() => null)
					: null;
				const guardAssignmentId =
					((guardFolder?.meta?.mode === 'assignment_writing' ||
						guardFolder?.meta?.category === 'assignment_project') &&
						guardFolder?.meta?.assignment_id) ||
					(chat?.meta?.category === 'assignment_workspace' && chat?.meta?.assignment_id);
				if (guardAssignmentId) {
					await goto(`/assignments/${guardAssignmentId}/write?chat=${chat.id}`);
					return false;
				}
			}
