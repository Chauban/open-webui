<script lang="ts">
	...
	const onDragOver = (e) => {
		e.stopPropagation();
		if (dragged || parentDragged || !isWritable || lockFolders) {
			return;
		}
		e.preventDefault();
		draggedOver = true;
		...
		<div class="w-full group">
			...
			>
				...
				{#if showVisibilityToggle}
					<button
						class="absolute z-10 right-2 hover-reveal self-center flex items-center rounded-full border border-gray-200 bg-white px-2 py-1 text-xs text-gray-600 dark:border-gray-700 dark:bg-gray-850 dark:text-gray-300"
						on:click={async (e) => {
							e.stopPropagation();
							e.stopImmediatePropagation();
				...
				{:else if isWritable && (!lockFolders || allowMenuWhenLocked)}
					<button
