...
<script lang="ts">
	...
	// RightWrite: writing projects route their chats into the writing workspace.
		...
		if (targetChatId) {
			...
			if (res) {
				if (targetChatId === get(chatId)) {
					if (projectId) {
						const folder = await getFolderById(localStorage.token, projectId).catch(() => null);
		...
		>
			<LinkPreview.Trigger
				...
				onclick={() => {
					...
					if (clearSelectedProjectOnChatClick && $selectedFolder) {
						selectedFolder.set(null);
					}
