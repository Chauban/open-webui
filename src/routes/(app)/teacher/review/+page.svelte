...
<TeacherPageShell title="Review">
	<div class="mx-auto max-w-6xl px-4 py-8">
		...
		<div class="mb-4 flex flex-wrap gap-2">
			...
			<button
				class={eduSegmentClass(selectedStatus === 'returned')}
				on:click={() => selectStatus('returned')}
			>
				{$i18n.t('Returned')}
			</button>
			<button class={eduSegmentClass(selectedStatus === 'all')} on:click={() => selectStatus('all')}>
				{$i18n.t('All')}
			</button>
		</div>

		<EduCard class="mb-8 grid gap-3 md:grid-cols-4">
			<select class={EDU_FIELD_CLASS} bind:value={selectedClassroom} on:change={() => loadQueue()}>
				{#each classroomOptions as option}
					<option value={option.value}>{option.label}</option>
				{/each}
			</select>
			<select class={EDU_FIELD_CLASS} bind:value={selectedAssignment} on:change={() => loadQueue()}>
						...
						<div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
							<div>
								...
								<div class="mt-3 flex flex-wrap gap-2 text-xs">
									...
									<EduBadge tone={getRiskTone(item)}>
