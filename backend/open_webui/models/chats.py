...
from open_webui.internal.db import Base, JSONField, get_async_db_context, get_db_context
...
class ChatTable:
    ...
    async def get_pinned_chats_by_user_id(
        ...
        async with get_async_db_context(db) as session:
            ...
            stmt = stmt.where(Chat.meta['internal'].as_boolean().is_not(True))
            # A writing session's chat belongs to its workspace, not the pinned chat list.
            stmt = stmt.filter(~exists().where(WritingSession.chat_id == Chat.id))
            result = await session.execute(stmt.order_by(Chat.updated_at.desc()))
            all_chats = result.all()
            return [
                ChatTitleIdResponse.model_validate(
