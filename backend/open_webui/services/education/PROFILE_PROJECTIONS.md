...
迁移 `e7b3d5f9a1c4` 把提交前反思改为教师按作业自定：证据载荷的 reflection 段换成题目快照（`evidence_schema_version` 2026-09-14.1），画像去掉 AI 帮助类型分布与生成/打磨迁移（`PROFILE_METRIC_VERSION` 2026-09-14.1）。同样要求画像证据表为空；作业与已交反思由迁移原样转写成原来那套固定题，不需要清理。
